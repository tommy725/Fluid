...
class PhoneLoginFragment : BaseFragment() {
    ...
    private val TAG = PhoneLoginFragment::class.java.simpleName
    ...
    fun loginUsingPassword(view: View) {
        view.hideKeyboard()
        (activity as MainActivity).loadPasswordLoginFragment(true)
    }
        ...
        FirebaseAuth.getInstance().signInWithCredential(credential)
            .addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    viewModel.progress.postValue(false)
                    viewModel.gotoAnimationFragment.postValue(true)
